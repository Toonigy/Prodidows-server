...
// --- Configuration ---
const SERVER_BASE_URL = "ws://localhost:10000"; // Base URL for WebSocket connections
const WORLD_LIST_PATH = "/game-api/world-list"; // Path for the central world list WebSocket
...
function showCustomError(title, message) {
    ...
    errorModal.innerHTML = `
        <div class="bg-gray-800 p-8 rounded-lg shadow-xl text-center max-w-sm mx-auto border border-red-500">
            ...
            <button id="errorModalCloseBtn" class="btn-primary">OK</button>
        </div>
    `;
...
function connectToWorldList() {
    ...
    worldListWs.onclose = () => {
        logMessage("Disconnected from world list WebSocket.", "system");
        worldListDiv.innerHTML = `<p class="text-center text-red-400">Could not connect to world list. Server might be down.</p>`;
    };

    worldListWs.onerror = (error) => {
        logMessage("World list WebSocket error: " + error.message, "error");
...
function setupWorldListUI(worlds) {
    ...
    worlds.forEach(world => {
        ...
        worldCard.className = "card flex flex-col items-center p-4 bg-gray-700 hover:bg-gray-600 cursor-pointer transition duration-200 ease-in-out transform hover:scale-105 rounded-xl";
        worldCard.innerHTML = `
            <h3 class="text-xl font-semibold text-indigo-300">${world.name}</h3>
            <p class="text-gray-400 text-sm mb-3">${world.path}</p>
            <button class="btn-primary mt-auto join-world-btn" data-world-path="${world.path}" data-world-name="${world.name}">
                Join World
            </button>
        `;
...
function joinWorld(path, name) {
    ...
    gameWorldWs.onclose = () => {
        logMessage(`Disconnected from world: ${name}.`, "system");
    ...
    gameWorldWs.onerror = (error) => {
        logMessage(`Game world WebSocket error for ${name}: ${error.message}`, "error");
...
function renderPlayers() {
    ...
    for (const id in playersInWorld) {
        ...
        playerDot.className = `player-dot ${id === userId ? 'local-player' : ''}`;
        playerDot.style.left = `${player.x}px`;
        playerDot.style.top = `${player.y}px`;
        playerDot.textContent = id === userId ? "You" : id.substring(0, 3); // Show "You" or first 3 chars
